...
(function($) {
	...
	var dkrtbsGmapsInit = function( fieldEl ) {
		...
		var mapOptions = {
			center:    new google.maps.LatLng( dkrtbsGmaps.defaults.latitude, dkrtbsGmaps.defaults.longitude ),
			zoom:      parseInt( dkrtbsGmaps.defaults.zoom ),
		...
		var markerOptions = {
			map: map,
			draggable: true,
			title: dkrtbsGmaps.strings.markerTitle
		};
	...
	dkrtbs.addCallbackForInit( function() {
		$('.dkrtbs_Gmap_Field .field-item').each(function() {
			dkrtbsGmapsInit( $(this) );
		});
	} );

	dkrtbs.addCallbackForClonedField( ['dkrtbs_Gmap_Field'], dkrtbsGmapsInit );
