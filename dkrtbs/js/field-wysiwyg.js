dkrtbs.addCallbackForDeletedField( 'dkrtbs_wysiwyg', function( el ) {

	// Destroy WYSIWYG editors instances.
	el.find( '.dkrtbs-wysiwyg textarea' ).each( function() {
...
dkrtbs.addCallbackForClonedField( 'dkrtbs_wysiwyg', function( newT ) {

	newT.find( '.dkrtbs-wysiwyg' ).each( function (i) {
		...
		nameRegex = new RegExp( 'dkrtbs-placeholder-name-' + fieldId, 'g' );
		idRegex   = new RegExp( 'dkrtbs-placeholder-id-' + fieldId, 'g' );

		// Placeholder markup for the new wysiwyg is stored as a prop on var dkrtbs_wysiwyg_editors
		// Copy, update ids & names & insert.
		el.html( dkrtbs_wysiwyg_editors[fieldId].replace( nameRegex, name ).replace( idRegex, id ) );

		// If no settings for this field. Clone from placeholder.
		if ( typeof( tinyMCEPreInit.mceInit[ id ] ) === 'undefined' ) {
			var newSettings = jQuery.extend( {}, tinyMCEPreInit.mceInit[ 'dkrtbs-placeholder-id-' + fieldId ] );
		...
		if ( typeof( tinyMCEPreInit.qtInit[ id ] ) === 'undefined' ) {
			var newQTS = jQuery.extend( {}, tinyMCEPreInit.qtInit[ 'dkrtbs-placeholder-id-' + fieldId ] );
...
dkrtbs.addCallbackForSortStart( 'dkrtbs_wysiwyg', function( el ) {
...
dkrtbs.addCallbackForSortEnd( 'dkrtbs_wysiwyg', function( el ) {
