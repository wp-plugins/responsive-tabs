...
abstract class dkrtbs_Field {
	...
	public function __construct( $name, $title, array $values, $args = array() ) {
		...
		if ( ! empty( $this->args['std'] ) && empty( $this->args['default'] ) ) {
			...
			_deprecated_argument( 'dkrtbs_Field', '0.9', "field argument 'std' is deprecated, use 'default' instead" );
		}
	...
	public function get_default_args() {
		return apply_filters(
			...
			array(
				...
				'data_delegate'       => null,
				'save_callback'       => null,
				'string-repeat-field' => __( 'Add New', 'dkrtbs' ),
				'string-delete-field' => __( 'Remove Field', 'dkrtbs' ),
			),
			get_class( $this )
		);
	}
	...
	public function get_the_id_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_id = preg_replace( '/dkrtbs\-field\-(\d+|x)/', 'dkrtbs-group-$1', $this->parent->get_the_id_attr() );
			$id = $parent_id . '[' . $id . ']';
		}

		$id .= '-dkrtbs-field-' . $this->field_index;
	...
	public function get_the_name_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_name = preg_replace( '/dkrtbs\-field\-(\d+|x)/', 'dkrtbs-group-$1', $this->parent->get_the_name_attr() );
			$name = $parent_name . '[' . $name . ']';
		}

		$name .= "[dkrtbs-field-$this->field_index]";
	...
	public function description() {
		...
		if ( ! empty( $this->args['desc'] ) ) { ?>
			...
			<div class="dkrtbs_metabox_description">
	...
	public function display() {
		...
		foreach ( $values as $key => $value ) {
			...
			<?php if ( $this->args['repeatable'] ) : ?>
				<button class="dkrtbs-delete-field" title="<?php echo esc_attr( $this->args['string-delete-field'] ); ?>">
					<span class="dkrtbs-delete-field-icon">&times;</span>
				</button>
			<?php endif; ?>
 ...
 * Standard text field.
 *
 * @extends dkrtbs_Field
 */
class dkrtbs_Text_Field extends dkrtbs_Field {
...
class dkrtbs_Text_Small_Field extends dkrtbs_Text_Field {

	public function html() {

		$this->args['class'] .= ' dkrtbs_text_small';
 ...
 */
class dkrtbs_File_Field extends dkrtbs_Field {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'dkrtbs-file-upload', trailingslashit( dkrtbs_URL ) . 'js/file-upload.js', array( 'jquery', 'dkrtbs-scripts' ) );
	...
	public function html() {
		...
		<div class="dkrtbs-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>

			<div class="dkrtbs-file-wrap-placeholder"></div>

			<button class="button dkrtbs-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'dkrtbs' ); ?>
			</button>

			<div class="dkrtbs-file-holder type-file <?php echo $this->get_value() ? '' : 'hidden'; ?>">

				<?php if ( $this->get_value() ) : ?>

					<?php if ( isset( $icon_img ) ) echo $icon_img; ?>

					<div class="dkrtbs-file-name">
			...
			<button class="dkrtbs-remove-file button <?php echo $this->get_value() ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Remove', 'dkrtbs' ); ?>
			</button>

			<input type="hidden"
				<?php $this->class_attr( 'dkrtbs-file-upload-input' ); ?>
				<?php $this->name_attr(); ?>
...
class dkrtbs_Image_Field extends dkrtbs_File_Field {
	...
	public function html() {
		...
		<div class="dkrtbs-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">

			<div class="dkrtbs-file-wrap-placeholder" style="<?php echo esc_attr( $placeholder_styles ); ?>">
			...
			<button class="button dkrtbs-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'dkrtbs-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'dkrtbs' ); ?>
			</button>

			<div class="dkrtbs-file-holder type-img <?php echo $this->get_value() ? '' : 'hidden'; ?>" data-crop="<?php echo (bool) $size['crop']; ?>">
	...
	static function request_image_ajax_callback() {
		...
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'dkrtbs-file-upload-nonce' ) ) )
			return;
...
add_action( 'wp_ajax_dkrtbs_request_image', array( 'dkrtbs_Image_Field', 'request_image_ajax_callback' ) );

/**
 * Standard text meta box for a URL.
 *
 */
class dkrtbs_URL_Field extends dkrtbs_Field {

	public function html() { ?>

		<input type="text" <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_url code' ); ?> <?php $this->name_attr(); ?> value="<?php echo esc_attr( esc_url( $this->value ) ); ?>" />
 ...
 *
 */
class dkrtbs_Date_Field extends dkrtbs_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_style( 'dkrtbs-jquery-ui', trailingslashit( dkrtbs_URL ) . 'css/vendor/jquery-ui/jquery-ui.css', '1.10.3' );

		wp_enqueue_script( 'dkrtbs-datetime', trailingslashit( dkrtbs_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'dkrtbs-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_small dkrtbs_datepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />

	<?php }
}

class dkrtbs_Time_Field extends dkrtbs_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'dkrtbs-timepicker', trailingslashit( dkrtbs_URL ) . 'js/jquery.timePicker.min.js', array( 'jquery', 'dkrtbs-scripts' ) );
		wp_enqueue_script( 'dkrtbs-datetime', trailingslashit( dkrtbs_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'dkrtbs-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_small dkrtbs_timepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>"/>
 ...
 *
 */
class dkrtbs_Date_Timestamp_Field extends dkrtbs_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_small dkrtbs_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
 ...
 *
 */
class dkrtbs_Datetime_Timestamp_Field extends dkrtbs_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_small dkrtbs_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_text_small dkrtbs_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
 ...
 */
class dkrtbs_Textarea_Field extends dkrtbs_Field {
 ...
 *  - int "rows" - number of rows in the <textarea>
 */
class dkrtbs_Textarea_Field_Code extends dkrtbs_Textarea_Field {
 ...
 *
 */
class dkrtbs_Color_Picker extends dkrtbs_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_script( 'dkrtbs-colorpicker', trailingslashit( dkrtbs_URL ) . 'js/field.colorpicker.js', array( 'jquery', 'wp-color-picker', 'dkrtbs-scripts' ) );
		wp_enqueue_style( 'wp-color-picker' );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'dkrtbs_colorpicker dkrtbs_text_small' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->get_value() ); ?>" />
 ...
 */
class dkrtbs_Radio_Field extends dkrtbs_Field {
 ...
 *
 */
class dkrtbs_Checkbox extends dkrtbs_Field {
 ...
 *
 */
class dkrtbs_Title extends dkrtbs_Field {
 ...
 *
 */
class dkrtbs_wysiwyg extends dkrtbs_Field {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'dkrtbs-wysiwyg', trailingslashit( dkrtbs_URL ) . 'js/field-wysiwyg.js', array( 'jquery', 'dkrtbs-scripts' ) );
	}
	...
	public function html() {
		...
		printf( '<div class="dkrtbs-wysiwyg" data-id="%s" data-name="%s" data-field-id="%s">', $id, $name, $field_id );

		if ( $this->is_placeholder() ) 	{

			// For placeholder, output the markup for the editor in a JS var.
			ob_start();
			$this->args['options']['textarea_name'] = 'dkrtbs-placeholder-name-' . $field_id;
			wp_editor( '', 'dkrtbs-placeholder-id-' . $field_id, $this->args['options'] );
			$editor = ob_get_clean();
			...
			<script>
				if ( 'undefined' === typeof( dkrtbs_wysiwyg_editors ) )
					var dkrtbs_wysiwyg_editors = {};
				dkrtbs_wysiwyg_editors.<?php echo $field_id; ?> = '<?php echo $editor; ?>';
			</script>
 ...
 */
class dkrtbs_Select extends dkrtbs_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2', trailingslashit( dkrtbs_URL ) . 'js/vendor/select2/select2.js', array( 'jquery' ) );
		wp_enqueue_script( 'field-select', trailingslashit( dkrtbs_URL ) . 'js/field.select.js', array( 'jquery', 'select2', 'dkrtbs-scripts' ) );
	}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2', trailingslashit( dkrtbs_URL ) . 'js/vendor/select2/select2.css' );
	}
	...
	public function output_field() {
		...
		<select
			...
			<?php $this->class_attr( 'dkrtbs_select' ); ?>
			style="width: 100%"
		>
	...
	public function output_script() {
		...
		$options = wp_parse_args( $this->args['select2_options'], array(
			'placeholder' => __( 'Type to search', 'dkrtbs' ),
			'allowClear'  => true,
		) );
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				if ( 'undefined' === typeof( window.dkrtbs_select_fields ) )
					window.dkrtbs_select_fields = {};

				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				window.dkrtbs_select_fields[id] = options;
...
class dkrtbs_Taxonomy extends dkrtbs_Select {
 ...
 *     'multiple'     => bool whether multiple can be selected
 */
class dkrtbs_Post_Select extends dkrtbs_Select {
	...
	public function output_script() {
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				var options = window.dkrtbs_select_fields[id];
				...
				<?php if ( $this->args['use_ajax'] ) : ?>
					...
					var ajaxData = {
						action  : 'dkrtbs_post_select',
						post_id : '<?php echo intval( get_the_id() ); ?>', // Used for user capabilty check.
						nonce   : <?php echo json_encode( wp_create_nonce( 'dkrtbs_select_field' ) ); ?>,
...
function dkrtbs_ajax_post_select() {
	...
	if ( ! $nonce || ! wp_verify_nonce( $nonce, 'dkrtbs_select_field' ) || ! current_user_can( 'edit_post', $post_id ) ) {
...
add_action( 'wp_ajax_dkrtbs_post_select', 'dkrtbs_ajax_post_select' );
 ...
 */
class dkrtbs_Group_Field extends dkrtbs_Field {
	...
	function __construct() {
		...
		if ( ! empty( $this->args['fields'] ) ) {
			foreach ( $this->args['fields'] as $f ) {
				...
				$class = _dkrtbs_field_class_for_type( $f['type'] );
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				'fields'              => array(),
				'string-repeat-field' => __( 'Add New Tab', 'dkrtbs' ),
				'string-delete-field' => __( 'Remove Tab', 'dkrtbs' ),
			)
		);
	}
	...
	public function html() {
		...
		<?php if ( $this->args['repeatable'] ) : ?>
			<button class="dkrtbs-delete-field">
				<span class="dkrtbs-delete-field-icon">&times;</span>
				<?php echo esc_html( $this->args['string-delete-field'] ); ?>
			</button>
		<?php endif; ?>

		<?php dkrtbs_Meta_Box::layout_fields( $fields ); ?>
	...
	public function add_field( dkrtbs_Field $field ) {
		$field->parent = $this;
...
/**
 * Google map field class for dkrtbs
 *
 ...
 * Note
 */
class dkrtbs_Gmap_Field extends dkrtbs_Field {
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				...
				'default_zoom'        => '8',
				'string-marker-title' => __( 'Drag to set the exact location', 'dkrtbs' ),
			)
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'dkrtbs-google-maps', '//maps.google.com/maps/api/js?sensor=true&libraries=places' );
		wp_enqueue_script( 'dkrtbs-google-maps-script', trailingslashit( dkrtbs_URL ) . 'js/field-gmap.js', array( 'jquery', 'dkrtbs-google-maps' ) );

		wp_localize_script( 'dkrtbs-google-maps-script', 'dkrtbsGmaps', array(
			'defaults' => array(
