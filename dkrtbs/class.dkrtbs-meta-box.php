...
class dkrtbs_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'dkrtbs_init_fields', array( &$this, 'init_fields' ) );
		...
		add_action( 'dkrtbs_save_fields', array( &$this, 'save' ) );
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _dkrtbs_field_class_for_type( $field['type'] );
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'dkrtbs-scripts', trailingslashit( dkrtbs_URL ) . 'js/dkrtbs.js', array( 'jquery' ) );

		wp_localize_script( 'dkrtbs-scripts', 'dkrtbsData', array(
			'strings' => array(
				'confirmDeleteField' => __( 'Are you sure you want to delete this field?', 'dkrtbs' )
			)
		) );
	...
	function enqueue_styles() {
		...
		$suffix = dkrtbs_DEV ? '' : '.min';

		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'dkrtbs-styles', trailingslashit( dkrtbs_URL ) . "css/dist/dkrtbs$suffix.css" );
		else
			wp_enqueue_style( 'dkrtbs-styles', trailingslashit( dkrtbs_URL ) . 'css/legacy.css' );
	...
	function add_for_id( $display ) {
		...
		// Don't show dkrtbs if we can't identify ID of a post
	 ...
	 *
	 * @param  dkrtbs_Field[]  $fields
	 */
	static function layout_fields( array $fields ) { ?>

		<div class="dkrtbs_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<div class="dkrtbs-cell-<?php echo intval( $field->args['cols'] ); ?>">

						<div <?php echo implode( ' ', $attrs ); ?>>
							<?php $field->display(); ?>
						</div>

						<input type="hidden" name="_dkrtbs_present_<?php esc_attr_e( $field->id ); ?>" value="1" />
	...
	function strip_repeatable( $values ) {
		...
		foreach ( $values as $key => $value ) {
			...
			if ( false !== strpos( $key, 'dkrtbs-group-x' ) || false !==  strpos( $key, 'dkrtbs-field-x' ) )
				unset( $values[$key] );
	...
	function save( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_dkrtbs_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _dkrtbs_field_class_for_type( $field['type'] ) ) {
				do_action( 'dkrtbs_save_' . $field['type'], $field, $value );
			}
